"""Page shell for the The Seals MEG sales room.
...
No brand book has been supplied for The Seals; the palette below is taken
from the draft template as authored.
"""
...
ICON = _grab(r'<div class="jnode">(<svg.*?</svg>)<span class="jnum">',
             "the journey window icon svg")
...
if os.path.exists(LOGO_B64):
    ...
    if _b64:
        ...
        LOGO = ('<div class="logo-plate"><img class="brandmark" '
                f'src="data:image/png;base64,{_b64}" alt="The Seals" '
                'width="420" height="154" fetchpriority="high" decoding="sync"></div>')
        LOGO_MISSING = False


def journey(current):
    """Six windows. Completed steps clear; the current one still shows fog."""
    out = []
    for i, label in enumerate(BUCKET_LABELS, 1):
        ...
        out.append(
            ...
            f'<div class="jnode">{ICON}<span class="jnum">{i}</span></div>'
...
<span class="sub">THE SEALS · Mutual Evaluation Guide</span>
...
<span>THE SEALS · An EverSmith Brands Company</span></footer>
</body></html>
"""
