...
Run from anywhere:  python3 prism-sales-room/build/build.py
...
def check(pages):
    ...
    for path, html in sorted(pages.items()):
        ...
        nows = re.findall(r'<div class="jstep now" aria-current="step">.*?'
                          r'<div class="jnum">(\d)</div>', html, re.S)[:1]
        if nows != [str(stage)]:
        ...
        for needed, what in (('class="hero-wave"', "hero wave"),
            ...
            if needed not in html:
                fails.append(f"{path}: missing {what}")
        nodes = html.count('class="jcanvas"')
        if nodes != 6:
            fails.append(f"{path}: expected 6 journey jcanvas nodes, found {nodes}")
...
def write_index(pages):
    ...
    L.append(f"{len(ORDER)} self-contained HTML pages for Prism Specialties Mutual Evaluation Guide.\n")
