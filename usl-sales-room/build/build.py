...
Run from anywhere:  python3 usl-sales-room/build/build.py
...
# of them. U.S. Lawns is five: Antonio and Inter are variable (one face
# each), PT Serif is static (regular, bold, italic).
...
def check(pages):
    ...
    for path, html in sorted(pages.items()):
        ...
        nows = re.findall(r'<div class="jstep now" aria-current="step">\n'
                          r'<div class="jturf">.*?<div class="jnum">(\d)</div>',
                          html, re.S)
        if nows != [str(stage)]:
        ...
        # U.S. Lawns chrome: the wave, the logo plate, and six lawns each
        # carrying a mower.
        for needed, what in (('class="hero-wave"', "hero wave"),
                             ('class="logo-plate"', "logo plate")):
            if needed not in html:
                fails.append(f"{path}: missing {what}")
        for cls, want in (("jturf", 6), ("jlawn", 6), ("jmower", 6)):
            n = html.count(f'class="{cls}"')
            if n != want:
                fails.append(f"{path}: expected {want} journey {cls}, found {n}")
...
def write_index(pages):
    ...
    L.append(f"{len(ORDER)} self-contained HTML pages for the U.S. Lawns Mutual Evaluation Guide.\n")
