"""Page shell for the U.S. Lawns MEG sales room.

Same approach as the other brands: the CSS, the hero wave and the journey lawn
marker are extracted from spec/page-template.html at build time, so a redesign
is a drop-in. Every content component (gate, flag, btn, slide, table) has the
same class names across brands, so parse.py is shared.

Brand palette is the U.S. Lawns Brand Guidelines v1.1 (June 2025) set, as the
template documents it. The journey is a lawn: completed steps are mown, the
current step is half-mown with the mower on it, future steps stand tall.
"""
...
# One lawn marker: the turf svg plus the mower, byte-identical on all six
# steps in the template. The .jstep classes select mown / half-mown / tall.
TURF = _grab(r'<div class="jstep now"[^>]*>\s*(<div class="jturf">.*?</div>)\s*'
             r'<div class="jnum">', "the journey turf block")
...
if os.path.exists(LOGO_B64):
    ...
    if _b64:
        LOGO = ('<div class="logo-plate"><img class="brandmark" '
                f'src="data:image/png;base64,{_b64}" alt="U.S. Lawns" '
                'width="450" height="112" fetchpriority="high" decoding="sync"></div>')
        LOGO_MISSING = False


def journey(current):
    """Six lawns. Completed steps are mown; the current one is being mown."""
    out = []
    for i, label in enumerate(BUCKET_LABELS, 1):
        ...
        out.append(
            f'<div class="{cls}"{aria}>\n{TURF}\n'
            f'<div class="jnum">{i}</div>\n<div class="jlbl">{label}</div>\n'
...
<span class="sub">U.S. Lawns · Mutual Evaluation Guide</span>
...
<span>U.S. Lawns · An EverSmith Brands Company</span></footer>
</body></html>
"""
