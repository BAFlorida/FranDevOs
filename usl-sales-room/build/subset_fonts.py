#!/usr/bin/env python3
"""Subset the five MEG faces to the characters these pages use, then emit
...
a page embedded under a content policy - Antonio silently falls back to a
generic sans and the display type stops looking like the brand. Embedding
makes the pages genuinely self-contained.

Antonio, Inter and PT Serif are all SIL Open Font License, which permits
embedding; see fonts/NOTICE.txt.
"""
...
# (file, family, weight, style, keep weight axis)
FACES = [
    ("Antonio.woff2", "Antonio", "400 700", "normal", True),
    ("Inter.woff2", "Inter", "400 800", "normal", True),
    ("PTSerif-400.woff2", "PT Serif", "400", "normal", False),
    ("PTSerif-700.woff2", "PT Serif", "700", "normal", False),
    ("PTSerif-400i.woff2", "PT Serif", "400", "italic", False),
]
...
def main():
    ...
    for fname, family, weight, style, variable in FACES:
        ...
        if variable:
            # Keep the weight axis so intermediate weights stay real weights
            # rather than a browser-synthesised faux bold.
        ...
        total_before += before
        total_after += after
        print(f"  {family + ' ' + style:18s} {before:>7,}B -> {after:>7,}B")
    ...
    css = ("/* MEG type system, embedded. Subset by build/subset_fonts.py so the\n"
           ...
           "   build/fonts/NOTICE.txt. Regenerate after changing copy. */\n"
           + "\n".join(blocks))
