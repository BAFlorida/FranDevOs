...
CREATE OR REPLACE VIEW es08_kg_hospitality_ops AS
SELECT 'kg_hospitality_ops' AS cohort_id, record_id,
       'pre-franchise restaurant/kitchen/food-service background, any brand bought' AS matched_on,
...
FROM (SELECT DISTINCT p.record_id FROM persons p JOIN roles r USING(record_id)
  WHERE NOT r.is_franchise_role AND r.start_ym IS NOT NULL AND r.start_ym < p.fr_start_ym
    AND regexp_matches(r.title_norm || ' ' || r.company_norm,
        '\b(restaurant|culinary|chef|kitchen|food service|food and beverage|hospitality|catering|qsr)\b'));
