#!/usr/bin/env python3
"""Adapt the 1-Tom MEG copy to MilliCare.
...
DST = pathlib.Path("/home/user/FranDevOs/millicare-sales-room/content/pages")
...
SWAPS = [
    ...
    ('1-Tom-Plumber has no one-time setup fee. Technology fees are paid directly to the vendors.',
     '[PLACEHOLDER: confirm against Item 7 whether MilliCare has a one-time setup fee and how technology fees are billed.]'),
    ("[PLACEHOLDER: technology fee acknowledgment — the previous acknowledgment was wrong: EverSmith does not collect these fees. 1-Tom-Plumber's technology fees run $1,000 to $8,000 per month paid direct to the vendors, and the arrangement needs a working session with 1-Tom-Plumber before candidate-facing acknowledgment copy can be written.]",
     '[PLACEHOLDER: technology fee acknowledgment — the shared acknowledgment was wrong (EverSmith does not collect these fees; the 1-Tom fee range does not transfer). Confirm how MilliCare technology fees are billed and rewrite the acknowledgment.]'),
    ("1-Tom-Plumber", "MilliCare"),
    ("1TP", "MC"),
    # The welcome video is brand-specific; 1-Tom's exists, MilliCare' is not
    # yet recorded. The brand template carries this exact slot label, with the
    # brand name set the way the chrome sets it.
    ("[VIDEO: Welcome]",
     "[VIDEO SLOT — PENDING: MilliCare welcome — awaiting final cut]"),
    ...
    # 1-Tom's Brand Overview video is live (2026-08-24); MilliCare' is not
    ...
    ("[VIDEO: Brand Overview]",
     "[VIDEO SLOT — PENDING: MilliCare Brand Overview]"),
]
