...
Run from anywhere:  python3 millicare-sales-room/build/build.py
...
def check(pages):
    ...
    for path, html in sorted(pages.items()):
        ...
        for needed, what in (('class="hero-wave"', "hero wave"),
            ...
            if needed not in html:
                fails.append(f"{path}: missing {what}")
        nodes = html.count('class="jfloor"')
        if nodes != 6:
            fails.append(f"{path}: expected 6 journey jfloor nodes, found {nodes}")
...
def write_index(pages):
    ...
    L.append(f"{len(ORDER)} self-contained HTML pages for MilliCare Mutual Evaluation Guide.\n")
