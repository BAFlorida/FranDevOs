# SOURCE COPY — Bucket 3, Validation

Original MEG language, brand-swapped to MilliCare. Render this copy as written.

---

## PAGE: 03-validation/01-owner-calls.html

**Eyebrow:** Stage 3 · Step 1 of 4
**H1:** Owner Calls
**Lede:** One of the most exciting steps in the Mutual Evaluation Process is connecting with owners.

### BODY

[VIDEO: EverSmith Validation Overview]

With your Qualification Summary submitted, introductions are made. We host conference calls with owners and prospective candidates.

What our owners tell you carries more weight than anything we have said. That is by design. Owner calls and funding run in tandem — work them both with your Franchise Developer rather than waiting on one to finish the other.

### H2: Your Call Link

Owner calls run on Microsoft Teams. The button below is your meeting link — it is the one thing you need from this page at call time, so keep it handy and open it straight from your phone.

[LINK: Join Your Owner Call — Microsoft Teams | owner-call-teams]

[LINK: Add to Calendar | owner-call-calendar]

### GATE — acknowledgment, required, blocking

**Label:** Required acknowledgment

**Statement:** I understand that my Franchise Developer can only speak to revenue and profitability numbers stated in the Franchise Disclosure Document Item 19. Any other information regarding aspects of financial performance must be obtained through conversations with franchise owners.

**Checkbox label:** I understand

*Render above the fold. This gate must block advancement.*


---

## PAGE: 03-validation/02-funding.html

**Eyebrow:** Stage 3 · Step 2 of 4
**H1:** Funding
**Lede:** Four paths, and a plan that has to hold together.

### BODY

[VIDEO: EverSmith Funding Overview]

There are four common paths to funding a franchise. An SBA loan, a ROBS rollover of pre-tax retirement funds, personal savings, or home equity.

Your Franchise Developer's job is to make sure you have a sound financial plan. We will introduce you to trusted third-party lenders who specialize in franchise funding.

**MilliCare will not award franchises to people who may be put in financial peril as a result.**

That is not a formality. It is a filter we apply on purpose.

### GATE — select, required

**Label:** Required — self assessment

**Statement:** What are your combined total Personal Monthly Household Responsibilities?

**Options:** Less than $1,000 · $1,000 - $2,999 · $3,000 - $4,999 · $5,000 - $7,999 · $8,000 - $11,999 · $12,000 or more

### GATE — select, required

**Label:** Required — self assessment

**Statement:** What percentage of those expenses is covered by outside sources?

*Outside sources include spouse income, rental income, and any existing business or personal income that will continue after you open your franchise.*

**Options:** 0 - 20% · 21 - 40% · 41 - 60% · 61 - 80% · 81 - 100%

### H2: SBA Funding

The two relevant programs are SBA Express and SBA 7(a). Our funding partners will explain the differences and walk you through pre-approval requirements.

Every funding partner requires a Personal Financial Statement.

[DOWNLOAD: SBA Personal Financial Statement — Fillable PDF]

**Who must file a PFS:**
- The proprietor, if a sole proprietorship
- Each general partner, if a partnership
- Each managing member, if an LLC
- Any owner of 20% or more of the business
- Any person or entity providing a guaranty on the loan

Your spouse must sign the PFS even if they are not on the loan.

[VIDEO: SBA 7(a) Loan Explainer]

### H2: ROBS Funding

ROBS, Rollover as Business Startup, lets you use pre-tax retirement funds to capitalize your business without early withdrawal penalties. It can move as fast as 10 days.

**STEPS:**
1. Set up a C Corporation.
2. Design a new qualified retirement plan for that corporation.
3. Transfer your existing retirement funds into the new plan.
4. The plan purchases stock in the new C Corporation, capitalizing the business.

[VIDEO: 401(k) Rollover Explainer]

### H2: Self-Funding

If you are funding with cash, send your Franchise Developer a current bank or investment statement as proof of funds.

[VIDEO: EverSmith Self Funding Overview]

### H2: Home Equity

A HELOC uses the equity in your home. The video below covers your options and the tradeoffs.

[VIDEO: EverSmith Home Equity Funding]

### NEXT
Confirm your funding path with your Franchise Developer, then continue to Territory.

---

## PAGE: 03-validation/03-territory.html

**Eyebrow:** Stage 3 · Step 3 of 4
**H1:** Territory
**Lede:** The decision that compounds for the life of your business.

### BODY

[VIDEO SLOT — PENDING: Territory and Premises]

### H2: Territory Design

Different brands prioritize different criteria. Some design around population. Some design around households. Recurring-service brands weight route density.

Your Franchise Developer designs your territory to maximize the potential of the business, not to hit a number on a page, and maps it with our platform team before anything is confirmed.

[PLACEHOLDER: MilliCare territory design criteria. Sister brands weight very different variables - consumer search density in one, commercial site count in another - so neither transfers. Supply the variable this brand's territories are actually drawn around, and the unit they are counted in.]

### H2: Your Premises

[PLACEHOLDER: premises requirement for MilliCare - whether a physical location is required, what type, and the size range. Sister brands differ - one allows a home address, another requires light-industrial flex. Confirm which applies here.]

### H2: You Will Need an Address

[PLACEHOLDER: confirm the address requirement for MilliCare before publishing this section. The gate below is carried over from the shared MEG language and must not go live until the requirement is confirmed to apply to this brand.]

Your Franchise Developer coordinates with our Local Marketing Specialist before your territory is finalized. You will form a legal entity under a name of your choosing, that entity name cannot include the brand name, and your public-facing name is your DBA.

[PLACEHOLDER: confirm MilliCare DBA naming convention and provide a worked example.]

### GATE — acknowledgment, required

**Label:** Required acknowledgment

**Statement:** I understand that I will need a unique physical address within my territory, and that P.O. boxes and shared mailbox services do not qualify.

**Checkbox label:** I understand

### NEXT
Continue to Understanding Your Brand's Technology.

---

## PAGE: 03-validation/04-technology.html

**Eyebrow:** Stage 3 · Step 4 of 4
**H1:** How Technology and Sourcing Works
**Lede:** The platforms you will run the business on, and what they cost.

### BODY

EverSmith Strategic Partnerships sources our software platforms, as you learned in Additional Brand Specific Resources.

### H2: Strategic Partnerships Across the Portfolio

We are constantly adding strategic partnerships across the EverSmith portfolio. The point is simple: leverage our scale across the entire portfolio to save our franchisees money with any and all vendors their business needs — insurance, marketing, 401(k), uniforms, compliance, payroll, bookkeeping, and everything in between.

### H2: Your Package

**TABLE:**
| Platform | Function |
| ServiceTitan | Customer and technician management, dispatch, scheduling |
| CareerPlug | Recruiting and applicant tracking |
| QuickBooks Online | Accounting and financial reporting, used with your Franchise Business Coach. A required system |
| FranConnect | Franchise management — onboarding, training, and communication with the brand team |

### H2: Your Platforms, Explained

[VIDEO: ServiceTitan Overview]

[VIDEO: EverSmith Career Plug]

[VIDEO: QuickBooks Online Overview]

[VIDEO SLOT — PENDING: FranConnect overview]

### H2: Pricing

Your technology pricing comes directly from Item 7 of the Franchise Disclosure Document — review the numbers there with your Franchise Developer. [PLACEHOLDER: confirm against Item 7 whether MilliCare has a one-time setup fee and how technology fees are billed.]

### NEXT
Continue to Stage 4, Seeking Approval.
